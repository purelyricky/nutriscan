...
export async function POST(req: Request) {
  try {
    ...
    console.log("OpenAI API: First file data format:", firstFile.substring(0, 50));

    // Convert data URL to Buffer (OpenAI works with Buffer)
    const base64Data = firstFile.split(',')[1];
    const fileBuffer = Buffer.from(base64Data, 'base64');

    console.log("OpenAI API: Buffer created, size:", fileBuffer.length);
    ...
    const result = streamObject({
      ...
      onFinish: ({ object, error }) => {
        console.log("OpenAI API: Stream finished, validating result");
        ...
        if (!object) {
          console.error("OpenAI API: No object returned from AI model");
          return;
        }

        const res = extractionResultSchema.safeParse(object);
        if (res.error) {
          console.error("OpenAI API: Validation error:", res.error);
          console.error("OpenAI API: Received object:", JSON.stringify(object, null, 2));
        } else {
    ...
    console.log("OpenAI API: Starting stream response");
    return result.toTextStreamResponse();
  } catch (error) {
    console.error("OpenAI API: Error in POST handler:", error);
